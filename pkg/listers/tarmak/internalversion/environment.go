...
type EnvironmentLister interface {
	...
	List(selector labels.Selector) (ret []*tarmak.Environment, err error)
...
// List lists all Environments in the indexer.
func (s *environmentLister) List(selector labels.Selector) (ret []*tarmak.Environment, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*tarmak.Environment))
	})
	return ret, err
}
...
type EnvironmentNamespaceLister interface {
	...
	Get(name string) (*tarmak.Environment, error)
...
// List lists all Environments in the indexer for a given namespace.
func (s environmentNamespaceLister) List(selector labels.Selector) (ret []*tarmak.Environment, err error) {
...
// Get retrieves the Environment from the indexer for a given namespace and name.
func (s environmentNamespaceLister) Get(name string) (*tarmak.Environment, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(tarmak.Resource("environment"), name)
	}
	return obj.(*tarmak.Environment), nil
}
