...
type WingJobLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.WingJob, err error)
...
// List lists all WingJobs in the indexer.
func (s *wingJobLister) List(selector labels.Selector) (ret []*v1alpha1.WingJob, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.WingJob))
	})
	return ret, err
}
...
type WingJobNamespaceLister interface {
	...
	Get(name string) (*v1alpha1.WingJob, error)
...
// List lists all WingJobs in the indexer for a given namespace.
func (s wingJobNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.WingJob, err error) {
...
// Get retrieves the WingJob from the indexer for a given namespace and name.
func (s wingJobNamespaceLister) Get(name string) (*v1alpha1.WingJob, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("wingjob"), name)
	}
	return obj.(*v1alpha1.WingJob), nil
}
