...
type PuppetTargetLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.PuppetTarget, err error)
...
// List lists all PuppetTargets in the indexer.
func (s *puppetTargetLister) List(selector labels.Selector) (ret []*v1alpha1.PuppetTarget, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.PuppetTarget))
	})
	return ret, err
}
...
type PuppetTargetNamespaceLister interface {
	...
	Get(name string) (*v1alpha1.PuppetTarget, error)
...
// List lists all PuppetTargets in the indexer for a given namespace.
func (s puppetTargetNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.PuppetTarget, err error) {
...
// Get retrieves the PuppetTarget from the indexer for a given namespace and name.
func (s puppetTargetNamespaceLister) Get(name string) (*v1alpha1.PuppetTarget, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("puppettarget"), name)
	}
	return obj.(*v1alpha1.PuppetTarget), nil
}
