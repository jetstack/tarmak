...
dev-root-token
EOS
    writer.add_file("etc/vault/vault-test", 0644) do |f|
      f.write(token)
    end
    writer.add_file("etc/vault/vault-unseal-0", 0644) do |f|
      f.write(token)
    end
    ...
    writer.add_file("home/vagrant/.ssh/id_rsa", 0644) do |f|
	    f.write(ssh_insecure_key)
    end
    writer.add_file("root/.ssh/id_rsa", 0644) do |f|
	    f.write(ssh_insecure_key)
    end
  end
  ...
  on host, "chmod 0600 /home/vagrant/.ssh/id_rsa"
  on host, "chmod 0600 /root/.ssh/id_rsa"
end

if hosts.length == 3
  describe '::vault_server' do
    context 'test three node vault_server cluster' do
...
class{'vault_server':
  ...
  volume_id => 'vol',
        ...
        hosts_as('vault_server').each do |host|
